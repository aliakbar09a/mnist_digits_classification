...
board = np.zeros((220, 220), dtype='uint8')
...
while(cap.isOpened()):
    ...
    cv2.rectangle(frame, (400, 50), (600, 250), (100, 100, 255), 1)
    ...
    for i in range(1, len(center_points)):
        if math.sqrt((center_points[i-1][0] - center_points[i][0])**2 +
            ...
            cv2.line(board, (center_points[i-1][0]+10, center_points[i-1][1]+10),
                     (center_points[i][0]+10, center_points[i][1]+10), 255, 5, cv2.LINE_AA)
    ...
    if np.max(board) != 0:
        ...
        prediction1 = np.argmax(LRmodel.predict(LR_input, dr.weights, dr.base))
    ...
    cv2.putText(frame, predict1_text,
                (5, 400), cv2.FONT_HERSHEY_PLAIN, 2, (255, 200, 200), 2, cv2.LINE_AA)
    cv2.putText(frame, predict2_text,
                (5, 460), cv2.FONT_HERSHEY_PLAIN, 2, (255, 200, 200), 2, cv2.LINE_AA)
